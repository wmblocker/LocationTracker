"use strict";
import LocationTracker = require('./types');
...
let currentUser: LocationTracker.User = {};
...
let locationsTable: Microsoft.WindowsAzure.MobileServiceTable;
...
let locationsArray: Array<LocationTracker.LocationsTable> = [];
let map: google.maps.Map;
let admobId: LocationTracker.Ad;
let tracker: NodeJS.Timer;
let tempTracker: NodeJS.Timer;
...
function initializeAd(): void {
    ...
    if (/(android)/i.test(navigator.userAgent)) { // for android & amazon-fireos
        admobId = {
            banner: 'ca-app-pub-4672297660394369/5324702239', // or DFP format "/6253334/dfp_example_ad"
...
function agreeToTerms(): void {
    navigator.notification.confirm(
        "You must read and agree to the terms and conditions before using the Location Tracker.",
...
function startLogin(): void {
    if (localStorage.getItem("terms") == null || localStorage.getItem("terms") == "false") {
        ...
        if (checkConnection() == true) {
            if (deviceFlag == false) {
                navigator.notification.confirm(
                    "Once logged in, this device and all locations will be bound to your account. Do you wish to continue?",
                    function (buttonIndex) {
                        if (buttonIndex == 1) {
                            loginGoogle();
                        }
        ...
        .then(function (results) {
            ...
            for (i = 0; i < results.length; i++) {
                ...
                approveBtn.onclick = function () {
                    navigator.notification.confirm(
                        "Are you sure you want to grant " + requestFrom + " permission to monitor your locations?",
                        function (buttonIndex) {
                            if (buttonIndex == 1) {
                ...
                denyBtn.onclick = function () {
                    navigator.notification.confirm(
                        "Are you sure you want to deny " + requestFrom + " permission to monitor your locations?",
                        function (buttonIndex) {
                            if (buttonIndex == 1) {
        ...
        .then(function (results) {
            for (i = 0; i < results.length; i++) {
                ...
                removeBtn.onclick = function () {
                    navigator.notification.confirm(
                        "Are you sure you want to stop " + grantedUser + " from having access to your locations?",
                        function (buttonIndex) {
                            if (buttonIndex == 1) {
...
function loadDeviceLocations(device: string) {
...
function startTempTracker(): void {
    var geocoder = new google.maps.Geocoder;
    var latlng;
...
function clearTempTracker(): void {
...
function showLocationsPage(): void {
    $("#devicesPage").fadeOut(function () {
        $("#locationsPage").fadeIn();
    });
}

function locationToDevice(): void {
    $("#locationsPage").fadeOut(function () {
        $("#devicesPage").fadeIn();
    });
}
...
function supportPrompt(): void {
    navigator.notification.alert(
        "If you have any questions about the Location Tracker, please contact our support team support@blockertech.com",  // message
        function () { },         // callback
        'Contact Us',            // title
        'Done'                  // buttonName
    );
